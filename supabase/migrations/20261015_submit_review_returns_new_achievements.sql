...
--
-- submit_review now also reports which achievement_key(s) (if any) were newly awarded by this
-- exact review, so the client can show a "you just earned X" modal on /study without a separate
-- round trip or a client-side snapshot diff.
--
-- award_achievement (20260924_kana_achievements.sql) is insert-only ("on conflict do nothing"),
-- and every achievement-evaluating function it feeds into (evaluate_kana_achievements,
-- evaluate_kanji_vocab_achievements) is invoked by an AFTER trigger on the exact progress table
-- submit_review itself updates below -- user_hiragana_progress, user_katakana_progress,
-- user_kanji_meaning_progress, user_kanji_reading_progress, user_vocabulary_progress. Postgres
-- runs AFTER ROW triggers synchronously, still inside this same function's transaction, so a
-- before/after snapshot of this user's user_achievements taken right around that one `update`
-- statement is exact -- no race window, no timestamp comparison, no second query after commit.
--
-- Body otherwise identical to submit_review's prior version (20261003_revert_sibling_rating_
-- mechanism.sql) -- only the achievement snapshot and the new output column were added.
...
create function public.submit_review(
 ...
 returns table(review_log_id bigint, resurfaces_today boolean, new_achievement_keys text[])
 language plpgsql
as $function$
declare
  v_current record;
...
begin
  ...
  select coalesce(array_agg(achievement_key), '{}') into v_achievements_before
  ...
  -- Achievement triggers on the table just updated above have already run by this point (AFTER
  -- ROW triggers fire before control returns from the update statement) -- anything new here was
  -- awarded by this exact review.
  select coalesce(array_agg(achievement_key), '{}') into v_new_achievements
    from public.user_achievements
    where user_id = p_user_id and achievement_key <> all(v_achievements_before);
