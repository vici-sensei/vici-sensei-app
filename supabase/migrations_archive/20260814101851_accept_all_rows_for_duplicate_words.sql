-- public.vocabulary.word is not unique: the same word can appear on multiple
-- rows (different senses/readings). When a student is quizzed on "What does
-- this word mean?" (vocab_meaning) or "How is this word read?" (kanji_reading),
-- they have no way of knowing which specific row the card was generated from,
-- so the meanings/readings of every row sharing that word must be accepted as
-- correct, not just the one row backing the card. get_due_cards now also
-- returns all_word_meanings / all_word_readings, aggregated across every
-- vocabulary row with the same word, for the client to check answers against.
