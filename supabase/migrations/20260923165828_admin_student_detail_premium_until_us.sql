-- US only. Mirror image of 20260923165827_admin_student_detail_premium_until_eu.sql (see its
-- header): mirror_eu instead of mirror_us, 'us'/'eu' labels swapped. mirror_eu.users already has
-- premium_until (20260923152732_premium_trial_admin_us.sql).
...
AS $$
  ...
  from (
    select id, display_name, email, avatar_url, country, is_premium, premium_until, created_at,
           pending_deletion_at, 'us'::text as region from public.users
    union all
    select id, display_name, email, avatar_url, country, is_premium, premium_until, created_at,
           pending_deletion_at, 'eu'::text as region from mirror_eu.users
  ) u
