...
function NotFound() {
  return (
    <div className="px-5 py-15 text-center text-text-muted">
      ...
      <Link href="/browse/vocabulary" prefetch={false} className={buttonClasses({ variant: "secondary", size: "sm", hover: "hover", className: "mt-4" })}>
        ← Back to results
      </Link>
    </div>
  );
...
function VocabularyDetailContent({ wordId }: { wordId: number }) {
  ...
  return (
    <div>
      <Link href="/browse/vocabulary" prefetch={false} className={buttonClasses({ variant: "secondary", size: "sm", hover: "hover", className: "mb-6" })}>
        ← Back to results
      </Link>
