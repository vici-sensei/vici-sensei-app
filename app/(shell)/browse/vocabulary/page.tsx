      ...
      {result.data.length === 0 ? (
        ...
        <>
          <div className="result-list">
            {result.data.map((row) => (
              <Link key={row.id} href={`/browse/vocabulary/${row.id}`} className="result-row">
                <div className="result-char" style={{ fontSize: "1.3rem", width: "auto", minWidth: 52 }}>
                  {row.word}
                </div>
                <div className="result-word-block">
                  <div className="result-main">{row.meanings?.join(", ")}</div>
                  <div className="result-sub">{row.kana_reading}</div>
                </div>
                <div className="lvl-badge">{row.jlpt_level ?? "—"}</div>
              </Link>
            ))}
          </div>
