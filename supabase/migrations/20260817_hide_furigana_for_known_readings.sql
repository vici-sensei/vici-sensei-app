-- On the kanji_reading ("Word reading") card, furigana is hidden for the
-- target kanji (see lib/study/furigana.tsx, renderTargetWord) but was shown
-- unconditionally for every other kanji in the word, even if the student has
-- already proven they know that kanji's specific reading elsewhere. This adds
-- a known_kanji_chars output column to get_due_cards so the client can also
-- hide furigana for "sibling" kanji whose reading is already mastered.
--
-- "Mastered" here means: for the (kanji_id, reading_group) pair backing that
-- sibling kanji in this word, the student has a user_kanji_reading_progress
-- row with status = 'review' (not 'relearning' -- a card that just lapsed
-- isn't mastered) and repetitions >= 2 (graduating once is too weak a
-- signal). This is stricter than the general "learned" definition used
-- elsewhere (status in ('review','relearning')) -- intentional, since a wrong
-- guess here breaks the reading-recall test, so the cost of a false positive
-- outweighs the cost of a false negative.
--
-- reading_group is only meaningful per kanji_id (see 20260811_use_reading_group_column.sql
-- and 20260812_kanji_word_reading_group_not_null.sql), so the sibling lookup
-- always joins back through kanji_word.id_kanji, never compares reading_group
-- across different kanji. vocabulary.ids_kanji is deliberately not used to
-- link word characters to kanji_id -- its ordering isn't guaranteed anywhere
-- in the code; kanji_word.id_word is the only reliable link, same as the
-- existing target-kanji lookup.
  ...
  from (
    ...
    select
      ...
      (
        ...
        from (
          select unnest(v2.meanings) as m from public.vocabulary v2 where v2.word = v.word
        ) meanings
