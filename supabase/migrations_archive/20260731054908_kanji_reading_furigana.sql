...
--
-- ReviewCardKanjiReading shows furigana above the kanji in the word that
-- *aren't* the one being tested (context help), so get_due_cards needs to
-- return vocabulary.furiganas for kanji_reading cards.
-- Postgres won't let CREATE OR REPLACE change a function's return columns,
-- so drop it first (mirrors 20260731_kanji_reading_input_data.sql).
