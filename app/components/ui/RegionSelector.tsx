...
import { SERVER_REGIONS, type ServerRegion } from "@/lib/serverRegion";
...
const ACCENT = {
  red: {
    selected: "border-accent-red bg-accent-red/[0.08] shadow-[0_0_20px_var(--color-accent-red-glow)]",
    iconSelected: "bg-accent-red text-white",
    checkSelected: "border-accent-red bg-accent-red text-white",
  },
  blue: {
    selected: "border-accent-blue bg-accent-blue/[0.08] shadow-[0_0_20px_rgba(0,210,255,0.35)]",
    iconSelected: "bg-accent-blue text-black",
    checkSelected: "border-accent-blue bg-accent-blue text-black",
  },
};
        ...
        return (
          <button
            ...
            className={`relative flex w-full items-center gap-4 rounded-2xl border p-4 text-left transition-all duration-200 disabled:cursor-not-allowed disabled:opacity-70 ${
              selected ? a.selected : "border-border-soft bg-white/[0.03] hover:border-white/20"
            }`}
          >
            <div
              className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-xl ${
                selected ? a.iconSelected : "bg-white/[0.06] text-text-muted"
              }`}
            >
              <Icon />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                {recommended !== undefined && option === recommended && (
            ...
            <span
              className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 ${
                selected ? a.checkSelected : "border-white/20 text-transparent"
              }`}
            >
