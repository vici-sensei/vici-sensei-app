-- Phase 6, part 3 of 3. EU only. New admin_* functions -- NONE of the existing self-service
-- functions (get_retention_rate, get_review_streak_record, get_student_daily_activity,
-- get_student_new_card_progress, get_admin_student_roster, get_admin_dashboard_stats,
-- get_admin_student_streaks) are touched. That's deliberate: several of those are NOT
-- SECURITY DEFINER and rely entirely on RLS on public.* for their access control (e.g.
-- lib/data/studyStats.ts calls get_retention_rate/get_review_streak_record for a student's OWN
-- stats). mirror_us has no RLS policies at all (by design, see the schema migration) -- pointing
-- one of those functions at admin_all instead of public would have let any authenticated caller
-- pass an arbitrary p_user_id and, if that id happened to belong to a mirrored US student, read
-- real data with no admin check at all. Every function below is new, SECURITY DEFINER, and starts
-- with an explicit is_admin() check -- fail closed regardless of what's granted on admin_all.
...
-- Consolidates fetchStudentDetail's 5-query Promise.all (users/leaderboard_stats/
-- user_study_settings/get_retention_rate/get_admin_student_streaks) into one round trip.
...
-- Content pools (kanji/vocabulary/hiragana/katakana) stay on public.* -- shared, identical
-- across both projects since Phase 1 (verified byte-for-byte).
...
-- Every category must carry all 5 ProgressStatus keys (new/learning/review/relearning/suspended),
-- zero-filled -- matching countByStatus() in lib/data/progress.ts, which the frontend type
-- (ProgressSummaryResponse) assumes. jsonb_object_agg over a bare group-by would silently omit a
-- status nobody has any rows in, which countByStatus's TS equivalent never does (it seeds all 5 to
-- 0 first) -- so the five-status cross join below is required, not just tidier.
...
-- Replaces fetchStudentActivityForDay's 5-table raw Promise.all (which relies on PostgREST's
-- FK-based nested embeds -- kanji:kanji_id(kanji) etc -- that only work for public.* tables;
-- mirror_us.review_logs/practice_logs deliberately have no FK to public.kanji/vocabulary/... so
-- those embeds can't be replicated for mirrored rows). Does the same joins server-side instead
-- and returns pre-labeled entries.
