...
--
-- The "Word reading" card (exercise_type kanji_reading, ReviewCardKanjiReading.tsx) wants to
-- show the example word's meaning, subtly, under the revealed correct reading. word_meanings
-- is already a column on both get_due_cards and get_kanji_intro_cards, but their kanji_reading
-- branches hardcode it to null -- unlike the vocab_meaning branch a few lines down, which
-- already selects it as `v.meanings as word_meanings` from the same joined `public.vocabulary v`.
-- Both functions' kanji_reading branches gain that same select instead of the null; no other
-- column, join, or filter changes.
