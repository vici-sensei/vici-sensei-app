-- EU only. Needs 20260923152640_premium_trial.sql applied to BOTH projects first (see its header).
-- US gets the mirror image, 20260923152732_premium_trial_admin_us.sql.
--
-- (1) premium_until into the US mirror. Same drift class 20260923051955_mirror_us_users_add_retired_to_region.sql
--     fixed: mirror_us.users is a `LIKE` snapshot and mirror_us_fdw.users a frozen IMPORT, and
--     mirror_us.refresh_all() copies users with a positional `SELECT *` -- both must gain the column
...
ALTER FOREIGN TABLE mirror_us_fdw.users ADD COLUMN premium_until timestamp with time zone;

ALTER TABLE mirror_us.users ADD COLUMN IF NOT EXISTS premium_until timestamp with time zone;
...
AS $$
  ...
  from (
    ...
    select id, display_name, email, avatar_url, country, is_premium, premium_until, stripe_customer_id,
           created_at, pending_deletion_at, 'eu'::text as region from public.users
    union all
    select id, display_name, email, avatar_url, country, is_premium, premium_until, stripe_customer_id,
           created_at, pending_deletion_at, 'us'::text as region from mirror_us.users
  ) u
...
BEGIN
  ...
  IF NOT FOUND THEN
    UPDATE mirror_us_fdw.users SET is_premium = p_is_premium, premium_until = v_until
    WHERE id = p_user_id AND NOT (is_premium AND stripe_customer_id IS NOT NULL);
    ...
    -- 5-minute mirror_us.refresh_all().
    UPDATE mirror_us.users SET is_premium = p_is_premium, premium_until = v_until WHERE id = p_user_id;
  END IF;
END;
$$;
