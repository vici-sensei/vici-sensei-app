...
--
-- Companion to 20261208_vocabulary_functions_use_primary_meanings.sql, which swapped these same 7
-- functions' data source from vocabulary.meanings to vocabulary.primary_meanings but kept the
-- output columns named `meanings`/`word_meanings`/`all_word_meanings` via aliasing, so no
-- TypeScript type had to change. At the user's request, this migration finishes the job: the
-- output columns themselves are renamed too, so the API contract stops lying about which column
-- backs it.
--
--   meanings           -> primary_meanings           (search_vocabulary, get_new_vocab_candidates,
--                                                      get_kanji_detail_words[_batch])
--   word_meanings       -> primary_word_meanings       (get_due_cards, get_kanji_intro_cards,
--                                                        complete_vocab_batch)
--   all_word_meanings   -> all_primary_word_meanings   (get_due_cards, complete_vocab_batch)
--
-- kanji_meanings (sourced from kanji.meanings, an unrelated table/column) is untouched throughout.
-- The application-side rename (lib/types, every component/hook reading these fields) lands in the
-- same commit as this migration.
--
-- Postgres refuses `create or replace function` when the RETURNS TABLE row type itself changes
-- (42P13: "cannot change return type of existing function"), unlike 20261208's swap which only
-- changed which column *fed* an unchanged output shape. Each function below is dropped first, then
-- recreated and re-granted `execute` to the same roles it already had it for (anon, authenticated,
-- service_role -- verified live before this migration, all three had it on all 7 functions).
...
drop function if exists public.get_kanji_intro_cards(uuid, bigint);
drop function if exists public.complete_vocab_batch(uuid);

create or replace function public.search_vocabulary(p_query text, p_level text[], p_limit integer, p_offset integer)
 returns table(id bigint, word text, kana_reading text, primary_meanings text[], parts_of_speech text[], ids_kanji bigint[], jlpt_level text, is_common_jisho boolean, usually_kana boolean, romaji_reading text, furiganas text[], romaji_furiganas text[], other_readings text[], total_count bigint)
 language sql
 stable
as $function$
  with params as (
  ...
  select id, word, kana_reading, primary_meanings, parts_of_speech, ids_kanji, jlpt_level, is_common_jisho,
...
stable
as $function$
  select v.id, v.word, v.kana_reading, v.primary_meanings, v.parts_of_speech, v.jlpt_level,
...
returns table(kanji_word_id bigint, reading_group integer, word text, kana_reading text, primary_meanings text[], furiganas text[], jlpt_level text)
language sql
stable
as $function$
  select kw.id, kw.reading_group, v.word, v.kana_reading, v.primary_meanings, v.furiganas, v.jlpt_level
...
stable
as $function$
  select kdw.kanji_id, kw.id, v.word, v.primary_meanings, v.jlpt_level, v.usually_kana, v.furiganas
  from public.kanji_detail_words kdw
  join public.kanji_word kw on kw.id = kdw.kanji_word_id
  join public.vocabulary v on v.id = kw.id_word
...
grant execute on function public.get_new_vocab_candidates(uuid, text[], integer) to anon, authenticated, service_role;
grant execute on function public.get_kanji_detail_words(bigint) to anon, authenticated, service_role;
grant execute on function public.get_kanji_detail_words_batch(bigint[]) to anon, authenticated, service_role;
grant execute on function public.get_due_cards(uuid, text[], boolean, boolean, boolean, boolean, integer) to anon, authenticated, service_role;
grant execute on function public.get_kanji_intro_cards(uuid, bigint) to anon, authenticated, service_role;
grant execute on function public.complete_vocab_batch(uuid) to anon, authenticated, service_role;
