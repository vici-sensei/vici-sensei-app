...
export async function fetchStudentRoster(supabase: AppSupabaseClient): Promise<StudentRosterRow[]> {
  const { data, error } = await supabase
    .from("users")
    .select(
      "id, display_name, email, avatar_url, country, is_premium, created_at, leaderboard_stats(current_streak, longest_streak, last_active_date, reviews_count, xp_points)"
    );
