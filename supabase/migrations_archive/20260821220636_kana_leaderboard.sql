...
--
-- Kana learning track, phase 8: leaderboard extension. review_logs is already
-- content-type agnostic, so hiragana_reading/katakana_reading reviews automatically
-- feed the "reviews" and "streak" metrics once submit_review starts writing them --
-- no changes needed there. "New cards" and "XP" aggregate per progress table
-- explicitly, so they need the two new tables added to their UNIONs, plus an
-- AFTER INSERT trigger on each reusing the existing leaderboard_stats_on_new_card()
-- (same function already used by user_kanji_meaning_progress/user_vocabulary_progress
-- -- it only reads NEW.user_id, so it works unchanged here).

create trigger leaderboard_stats_new_hiragana_trigger
  after insert on public.user_hiragana_progress
  for each row execute function leaderboard_stats_on_new_card();

create trigger leaderboard_stats_new_katakana_trigger
  after insert on public.user_katakana_progress
  for each row execute function leaderboard_stats_on_new_card();
