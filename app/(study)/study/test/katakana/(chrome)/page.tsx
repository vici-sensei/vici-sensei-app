...
/** Katakana reading test -- fixed word list, one attempt per word, shown one at a time. Both
 * outcomes are persisted (see user_reading_test_progress's doc comment), so a word stays locked
 * across a refresh/reopen once answered, right or wrong -- only the summary page's "Retry the
 * ones I got wrong" reopens a wrong one. Once every word in this pass has a result, advancing
 * past the last one redirects to the score screen. */
export default function KatakanaReadingTestPage() {
  ...
  // word still open when I arrived" (everything, on a first attempt; just the reopened wrong
  ...
  // before this visit (revisiting a locked/finished pass) or there are no words at all. Either
  // way there's no question to show, so redirect immediately: straight to the dashboard if that
  // prior pass had already reached 100% (nothing new to celebrate, stays locked), to the summary
  // otherwise.
  //
  ...
  useEffect(() => {
    ...
    window.location.href = "/study/test/katakana/summary";
  ...
  // Advances to the next word in this pass, or -- once every word in it has a result -- does the
  // same hard navigation to the summary the redirect effect above does, except this one only
  // fires from the student's own Next click on the LAST word, so they always get to see that
  // word's result before the page changes.
  const handleNext = () => {
  ...
  if (!currentSentence) {
    // Just answered the last word in this pass -- the redirect effect above is about to navigate
    // away.
    return <FullScreenLoader />;
  }
