...
--
-- get_due_cards took p_as_of timestamptz default now() -- a safe default, but
-- fetchFirstDueCard/fetchStudyQueue (lib/data/studyQueue.ts) explicitly overrode it with
-- new Date().toISOString(), so which cards count as "due" was actually decided by the
-- client's clock. Drop the parameter entirely; due_at <= now() is an absolute-instant
-- comparison so no timezone is needed here (unlike the "today" bucketing elsewhere).
--
-- Body is otherwise byte-identical to the current live definition (20260817_hide_furigana_for_known_readings.sql)
-- -- only the 3 `p.due_at <= p_as_of` occurrences become `p.due_at <= now()`.
...
grant execute on function public.get_due_cards(uuid, text[], boolean, boolean, integer) to authenticated;
