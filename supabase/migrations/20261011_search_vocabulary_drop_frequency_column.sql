-- Drops `frequency` from search_vocabulary's result shape ahead of the
-- column itself being dropped (see 20261012_drop_vocabulary_frequency_column.sql).
-- Nothing in the app reads this field: it was only ever carried through the
-- RPC result, never destructured on the frontend.
