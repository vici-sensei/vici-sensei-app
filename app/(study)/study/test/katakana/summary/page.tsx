...
function SummaryContent() {
  ...
  // words answered right back on attempt 1, which are locked and never re-asked (see
  // resetWrongAnswers). Showing those alongside a handful of just-reopened words would read as
  ...
  // forget markAnswered() for the last word is guaranteed to have reached the server (see the
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      // (pre-retry) instance whose progress still shows the just-reopened word as locked.
      window.location.href = "/study/test/katakana";
    } catch {
      setRetrying(false);
      showToast("Couldn't reopen those words — please try again.", "error");
    }
  };
...
export default function KatakanaReadingTestSummaryPage() {
  return (
